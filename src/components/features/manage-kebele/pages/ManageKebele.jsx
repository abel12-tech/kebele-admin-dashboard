...
import {
  ...
  useGetKebelesQuery,
} from "../../manage-admins/api/adminApi";

const ManageKebele = () => {
  const { isDarkMode, initializeDarkMode } = useDarkMode();
  const [currentPage, setCurrentPage] = useState(1);
  const { data: kebeles, isLoading, isSuccess, error } = useGetKebelesQuery();

  const [deleteKebele] = useDeleteKebeleMutation();
  ...
  const totalPages = Math.ceil((kebeles?.length || 0) / itemsPerPage);

  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = Math.min(startIndex + itemsPerPage, kebeles?.length || 0);
      ...
      <div className="flex flex-col flex-1 w-full">
        <div className="w-full container h-screen p-6 overflow-y-auto rounded-lg shadow-xs">
          <div className="flex justify-end mt-4 mb-4">
            <Link
              to="/add-kebele" // Update this path if needed
              className={`items ${
          ...
          <div className="w-full overflow-x-auto">
            <table className="w-full whitespace-no-wrap">
              <thead>
                ...
                >
                  <th className="px-4 py-3">Kebele Name</th>
              ...
              >
                {isLoading ? (
                  ...
                  kebeles.slice(startIndex, endIndex).map((kebele) => (
                    <tr
                      key={kebele._id}
                      className={`${
                    ...
                    >
                      <td className="px-4 py-3 text-sm">{kebele.name}</td>
          ...
          >
            <span className="flex items-center col-span-3">
              Showing {startIndex + 1}-{endIndex} of {kebeles?.length}
            </span>
