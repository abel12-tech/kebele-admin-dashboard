...
const ManageAdmins = () => {
  ...
  const { data: admins, isLoading, isSuccess, error } = useGetAdminsQuery();
  ...
  const endIndex = Math.min(startIndex + itemsPerPage, admins?.length || 0);
        ...
        <div className="w-full container h-screen p-6 overflow-y-auto rounded-lg shadow-xs">
            ...
            <table className="w-full whitespace-no-wrap">
              ...
              >
                {isLoading ? (
                  ...
                  admins.slice(startIndex, endIndex).map((admin) => (
                    <tr
                      key={admin._id}
                      className={`${
          ...
          >
            <span className="flex items-center col-span-3">
              Showing {startIndex + 1}-{endIndex} of {admins?.length}
            </span>
